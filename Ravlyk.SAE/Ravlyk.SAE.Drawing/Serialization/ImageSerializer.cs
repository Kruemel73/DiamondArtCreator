...
namespace Ravlyk.SAE.Drawing.Serialization
{
	...
	public static class ImageSerializer
	{
		...
		const string XmlDacVersion = "DACVERSION";
		...
		public static void WriteToXml(this CodedImage image, XmlWriter writer)
		{
			...
			writer.WriteSingleElement(XmlDacVersion, image.DacVersion);
			...
			while (reader.IsStartElement())
			{
				switch (reader.Name)
				{
					...
					case XmlSourceImageFileName:
						...
						break;
                    case XmlDacVersion:
                        image.DacVersion = reader.ReadElementContentAsString();
                        break;
		...
		/// <returns>New Image object.</returns>
		public static CodedImage LoadFromStream(Stream stream, string fileName)
		{
			if (stream != null)
			{
				try
				...
				{
					var msg = MessageBox.Show("Die Datei  ||" + fileName + "|| ist fehlerhaft !!!", "Error!", MessageBoxButtons.OK);
				}
			}
			return null;
		}
