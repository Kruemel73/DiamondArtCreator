...
namespace Ravlyk.SAE.Drawing.Processor
{
	public class ImageSymbolsController : ImageController<ImageSymbolsManipulator>
	{
		...
		public void AddRandomSymbolsWithoutImage(int count)
		{
			...
			var random = new Random();
			for (int i = SelectedCount; i < count; i++)
			{
				var nextIndex = random.Next(AvailableSymbols.Count);
				var newSymbol = GetFirstUnselectedSymbol(nextIndex, AvailableSymbols.Count);
				if (newSymbol == 0)
		...
		public void SetSymbolsAfterLoad(string[] result)
        {
			...
			for (int i = 0; i < resultLines; i++)
			{
                ...
                {
					var kv = AvailableSymbols[j];
					char Sym = char.Parse(result[i]);
					if (kv.Key == Sym)
                    {
						AvailableSymbols[j] = new KeyValuePair<char, bool>(kv.Key, true);
						break;
                    }
                }
            }
		...
		public void GetSymbolKey()
        {
			for (int i = 0; i < AvailableSymbols.Count; i++)
            {
				...
				if (kv.Value)
                {
					SAEWizardSettings.Default.ChoosenSymbol =  ((int)kv.Key);
                }
            }
		}
