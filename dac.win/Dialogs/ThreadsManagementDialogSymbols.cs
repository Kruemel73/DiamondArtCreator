...
{
	public partial class ThreadsManagementDialogSymbols : Form
	{
		public ThreadsManagementDialogSymbols()
		{
			InitializeComponent();
		}
		...
		void Initialize()
		{
			...
			gridViewThreads.Font = new Font("DiamondArtCreator", 10);

			if (File.Exists(Settings.Default.UserPalettesLocation + @"\" + "os.txt"))
			{
				ribbonButtonLoadSymbolsOS.Visible = true;
			}
			if (File.Exists(Settings.Default.UserPalettesLocation + @"\" + "osc.txt"))
			{
				ribbonButtonLoadSymbolsOSC.Visible = true;
			}

		}

		public string ChoosenSymbol;
				...
				if (editedColors != null)
				{
					...
					if (!HasChanges)
					{
						foreach (var newColor in editedColors)
						{
							...
							if (oldColor == null || oldColor.Argb != newColor.Argb || oldColor.ColorCode != newColor.ColorCode || oldColor.ColorName != newColor.ColorName || oldColor.SymbolChar != newColor.SymbolChar)
							{
								HasChanges = true;
								break;
							}
		...
		void gridViewThreads_CellClick(object sender, DataGridViewCellEventArgs e)
		{
			...
			if (e.ColumnIndex == ColumnSelectSymbol.Index && e.RowIndex >= 0)
			{
				var row = gridViewThreads.Rows[e.RowIndex];
				var color = row.DataBoundItem as CodedColor;
				if (color != null)
				{
					var colorsList = gridViewThreads.DataSource as IList<CodedColor>;
					using (var symbolsDialog = new SymbolManagerPerColor(colorsList))
					{
						//var colorsList = gridViewThreads.DataSource as IList<CodedColor>;
						symbolsDialog.ShowDialog(this);
						var newSymbol = Settings.Default.Symbol;
						var newColor = new CodedColor(color.R, color.G, color.B) { ColorCode = color.ColorCode, ColorName = color.ColorName, SymbolChar = color.SymbolChar };
						if (colorsList != null)
						{
							newColor.SymbolChar = (char)newSymbol;
							//var newSymbol = Settings.Default.Symbol;
							colorsList[e.RowIndex] = newColor;
							gridViewThreads.UpdateCellValue(ColumnSymbol.Index, e.RowIndex);
		...
		public bool InstalledVersion = Settings.Default.MobileSetup;

		private void SaveSymbols()
		{
			filename = Settings.Default.UserPalettesLocation + @"\" + currentPalette.Name + ".sym";
		...
		private void ribbonButtonLoadSymbols_Click(object sender, EventArgs e)
		{
			if (File.Exists(Settings.Default.UserPalettesLocation + @"\" + "os.txt"))
			{
			...
			// read file ox.txt and put entrys to result[]
			int breaking = 0;
			if (File.Exists(filename))
			{
				...
				foreach (var color in colorsList)
				{
					...
					if (Count < lineCount)
					{
						color.SymbolChar = Convert.ToChar(result[Count]);
					}
                    else
                    {
						return;
        ...
        private void ribbonButtonLoadSymbolsOSC_Click(object sender, EventArgs e)
        {
			if (File.Exists(Settings.Default.UserPalettesLocation + @"\" + "osc.txt"))
			{
			...
			if (File.Exists(filename))
			{
				...
				foreach (var color in colorsList)
				{
					if (Count < lineCount)
					{
						...
						if (color.ColorCode == result[Count, 0])
						{
							color.SymbolChar = Convert.ToChar(result[Count, 1]);
						}
						else
						{
							Count--;
