	...
	public static class AppInfo
	{
		public static string AppDescription
		{
			get
			{
				...
				return assembly?.GetCustomAttribute<AssemblyDescriptionAttribute>()?.Description ?? "Diamond Art Creator";
			}
		}
		...
		{
			Process.Start("http://www.diamondartcreator.de/");
		}

		public static void EmailToSupport()
		{
			Process.Start($"mailto:info@diamondartcreator.de?subject={AppDescription.Replace(" ", "%20")}%20({AppVersion})");
		}
		...
		{
			const string FileUrlTemplate = "http://www.diamondartcreator.de/Download/winver..txt";
			...
			if (string.IsNullOrEmpty(Settings.Default.Locale) || Settings.Default.Locale.Equals("de", StringComparison.InvariantCultureIgnoreCase))
			{
				checkForUpdatesEn();
			}
			else
